...
| ------- | ------------------------------------------------------------ | ------------------------------------------------------------------- |
| cargo   | `[workspace] members = [...]` (virtual or root-package)      | each member's `[package].version`; skipped when `[workspace.package].version` is used; intra-workspace dependency `version` requirements (including `[workspace.dependencies]`) are updated to the new version |
